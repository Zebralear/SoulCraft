...
public class PatienceC2S {
 public PatienceC2S() {
 	 }
 
 public PatienceC2S(FriendlyByteBuf buf) {
	 }
	 ...
	 context.enqueueWork(() -> {
		 ...
		 player.getCapability(SoulHasProvider.Soul_Has).ifPresent(soulHas -> {
			 ...
			 if(SoulCheck != 1) {
				 soulHas.addSoul(1);
				 soulType.setType(16);
				 Minecraft.getInstance().player.sendSystemMessage(Component.literal("You are filled with Patience!"));
			 } else {
