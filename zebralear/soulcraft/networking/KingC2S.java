...
public class KingC2S {
 public KingC2S() {
 	 }
 
 public KingC2S(FriendlyByteBuf buf) {
	 }
	 ...
	 context.enqueueWork(() -> {
		 ...
		 player.getCapability(SoulHasProvider.Soul_Has).ifPresent(soulHas -> {
			 ...
			 if(SoulCheck != 1) {
				 soulHas.addSoul(1);
				 soulType.setType(7);
				 Minecraft.getInstance().player.sendSystemMessage(Component.literal("Despite all you've done, you are ready for combat."));
			 } else {
