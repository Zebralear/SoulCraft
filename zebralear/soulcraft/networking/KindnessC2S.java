...
public class KindnessC2S {
 public KindnessC2S() {
 	 }
 
 public KindnessC2S(FriendlyByteBuf buf) {
	 }
	 ...
	 context.enqueueWork(() -> {
		 ...
		 player.getCapability(SoulHasProvider.Soul_Has).ifPresent(soulHas -> {
			 ...
			 if(SoulCheck != 1) {
				 soulHas.addSoul(1);
				 soulType.setType(15);
				 Minecraft.getInstance().player.sendSystemMessage(Component.literal("You are filled with Kindness!"));
			 } else {
