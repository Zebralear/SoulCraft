...
public class PerseveranceC2S {
 public PerseveranceC2S() {
 	 }
 
 public PerseveranceC2S(FriendlyByteBuf buf) {
	 }
	 ...
	 context.enqueueWork(() -> {
		 ...
		 player.getCapability(SoulHasProvider.Soul_Has).ifPresent(soulHas -> {
			 ...
			 if(SoulCheck != 1) {
				 soulHas.addSoul(1);
				 soulType.setType(17);
				 Minecraft.getInstance().player.sendSystemMessage(Component.literal("You are filled with Perseverance!"));
			 } else {
