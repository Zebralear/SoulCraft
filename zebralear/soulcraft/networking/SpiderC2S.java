...
public class SpiderC2S {
 public SpiderC2S() {
 	 }
 
 public SpiderC2S(FriendlyByteBuf buf) {
	 }
	 ...
	 context.enqueueWork(() -> {
		 ...
		 player.getCapability(SoulHasProvider.Soul_Has).ifPresent(soulHas -> {
			 ...
			 if(SoulCheck != 1) {
				 soulHas.addSoul(1);
				 soulType.setType(2);
				 Minecraft.getInstance().player.sendSystemMessage(Component.literal("You Spin a web of magic. This will surely be useful in combat"));
			 } else {
