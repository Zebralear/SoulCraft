...
public class SansC2S {
 public SansC2S() {
 	 }
 
 public SansC2S(FriendlyByteBuf buf) {
	 }
	 ...
	 context.enqueueWork(() -> {
		 ...
		 player.getCapability(SoulHasProvider.Soul_Has).ifPresent(soulHas -> {
			 ...
			 if(SoulCheck != 1) {
				 soulHas.addSoul(1);
				 soulType.setType(3);
				 Minecraft.getInstance().player.sendSystemMessage(Component.literal("You feel a profound lazyness wash over you. Perhaps you aren't ready for combat..."));
			 } else {
